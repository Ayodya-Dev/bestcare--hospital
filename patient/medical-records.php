...
$sql = "SELECT m.*, st.full_name AS doctor_name, st.specialization
...
<div class="pd-main">
    ...
    <main class="pd-content pr-page">
        <div class="pr-breadcrumb">
            <a href="dashboard.php">Dashboard</a> / <span>Medical Records</span>
        </div>
        ...
        <div class="pr-head">
            <div class="pr-head-left">
                <div>
                    ...
                    <p>Your visit history and diagnoses from BestCare doctors.</p>
                </div>
            </div>
            <div class="pr-head-right">
                <span class="pr-stat"><?php echo $count; ?> record<?php echo $count == 1 ? '' : 's'; ?></span>
                <span class="pr-sort">Sorted by: <strong>Newest First</strong></span>
            </div>
        </div>

        <?php if ($count > 0) { ?>
            <div class="pr-section-label">Chronological History</div>
            <div class="pr-list" id="prList">
                <?php for ($i = 0; $i < $count; $i++) {
                    $row = $rows[$i];
                    $notes = isset($row['notes']) ? $row['notes'] : '';
                    $search_blob = strtolower($row['diagnosis'] . ' ' . $row['doctor_name'] . ' ' . $notes);
                    ?>
                    <a class="pr-card" href="record-details.php?id=<?php echo (int)$row['id']; ?>" data-search="<?php echo htmlspecialchars($search_blob); ?>">
                        <div class="pr-date">
                            <span class="day"><?php echo date('D', strtotime($row['visit_date'])); ?></span>
                            <span class="full"><?php echo date('M d, Y', strtotime($row['visit_date'])); ?></span>
                        </div>
                        <div class="pr-body">
                            <span class="pr-badge">Visit Record</span>
                            <h3><?php echo htmlspecialchars($row['diagnosis']); ?></h3>
                            <p class="meta">
                            ...
                            ?>
                                <span class="pr-label">Notes</span>
        ...
        <?php } else { ?>
            <div class="pr-empty">
                No medical records yet. After a doctor visit, your records will appear here.
            </div>
        <?php } ?>
    </main>
</div>
