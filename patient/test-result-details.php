...
$sql = "SELECT * FROM test_results
        WHERE id = $test_id AND patient_id = $patient_id";
$result = mysqli_query($conn, $sql);

if (!$result) {
    bestcare_fail_page("Could not load test result. Please try again.");
}
...
<head>
    ...
    <title>Test Result Details - BestCare Hospital</title>
...
<aside class="pd-sidebar">
    ...
    <nav class="pd-nav" id="pdNav">
        ...
        <a class="active" href="test-results.php"><img src="<?php echo $dash; ?>/IMG_5.svg" alt=""> Test Results</a>
...
<div class="pd-main">
    <header class="pd-topbar">
        <form class="pd-search" method="get" action="test-results.php">
            <img src="<?php echo $dash; ?>/IMG_11.svg" alt="">
            <input type="text" name="q" placeholder="Search results..." autocomplete="off">
        </form>
    ...
    <main class="pd-content pr-page">
        <?php if ($test == false) { ?>
            <div class="pr-empty">
                Test result not found. <a href="test-results.php">Back to Test Results</a>
            </div>
        <?php } else { ?>
            ...
            <div class="prd-head">
                <a class="prd-back" href="test-results.php" title="Back">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>
                </a>
                <div>
                    <h1>Test Result</h1>
                    <p>Full lab / diagnostic findings from BestCare Hospital.</p>
                </div>
            </div>
            ...
            <div class="prd-card">
                <div class="prd-top">
                    ...
                    <span class="prd-date"><?php echo date('l, M d, Y', strtotime($test['result_date'])); ?></span>
                </div>

                <h2 class="prd-title"><?php echo htmlspecialchars($test['test_name']); ?></h2>

                <div class="prd-block">
                    <span class="prd-label">Result</span>
                    <p class="prd-text"><?php echo nl2br(htmlspecialchars($test['result_text'])); ?></p>
                </div>
            </div>
        <?php } ?>
    </main>
