...
$rx_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

$sql = "SELECT pr.*, st.full_name AS doctor_name, st.specialization, st.contact AS doctor_contact,
               d.name AS department_name
        FROM prescriptions pr, staff st, departments d
...
if (!$result) {
    bestcare_fail_page("Could not load prescription. Please try again.");
}

$rx = false;
if (mysqli_num_rows($result) > 0) {
    $rx = mysqli_fetch_array($result);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Prescription Details - BestCare Hospital</title>
...
<aside class="pd-sidebar">
    ...
    <nav class="pd-nav" id="pdNav">
        ...
        <a class="active" href="prescriptions.php"><img src="<?php echo $dash; ?>/IMG_5.svg" alt=""> Prescriptions</a>
        ...
        <a href="my-queries.php"><img src="<?php echo $dash; ?>/IMG_6.svg" alt=""> Queries</a>
...
<div class="pd-main">
    <header class="pd-topbar">
        <form class="pd-search" method="get" action="prescriptions.php">
            <img src="<?php echo $dash; ?>/IMG_11.svg" alt="">
            <input type="text" name="q" placeholder="Search prescriptions..." autocomplete="off">
        </form>
    ...
    <main class="pd-content pr-page">
        <?php if ($rx == false) { ?>
            <div class="pr-empty">
                Prescription not found. <a href="prescriptions.php">Back to Prescriptions</a>
            </div>
        <?php } else {
            $doc_name = $rx['doctor_name'];
        ...
        ?>
            ...
            <div class="prd-head">
                <a class="prd-back" href="prescriptions.php" title="Back">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>
                </a>
                <div>
                    <h1>Prescription</h1>
                    <p>Full medication details from your doctor.</p>
                </div>
            </div>
            ...
            <div class="prd-card">
                <div class="prd-top">
                    ...
                    <span class="prd-date"><?php echo date('l, M d, Y', strtotime($rx['issued_date'])); ?></span>
                </div>

                <h2 class="prd-title"><?php echo htmlspecialchars($rx['medication']); ?></h2>
                ...
                <div class="prd-doctor">
                    ...
                    <div>
                        ...
                        <p>
                            <?php echo htmlspecialchars($rx['specialization']); ?>
                            <?php if (!empty($rx['department_name'])) { ?>
                                · <?php echo htmlspecialchars($rx['department_name']); ?>
                            <?php } ?>
                        </p>
                    </div>
                </div>
                ...
                <div class="prd-block">
                    <span class="prd-label">Dosage</span>
                    <p class="prd-text"><?php echo nl2br(htmlspecialchars($rx['dosage'])); ?></p>
                </div>

                <?php if (!empty($rx['doctor_contact'])) { ?>
                    <div class="prd-block">
                        <span class="prd-label">Doctor Contact</span>
                        <p class="prd-text"><?php echo htmlspecialchars($rx['doctor_contact']); ?></p>
                    </div>
                <?php } ?>
            </div>
        <?php } ?>
